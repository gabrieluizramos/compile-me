...
>>>>>>> 323154d3a62c2291875218272ee8d78d59c2c173
...
- Qual a diferença de uma prop pra um state?
    - R: `Props` são valores recebidos como parâmetros ao instanciar um componente `React`. `State` são estados, ou seja, conjunto de definições, condições e valores determinados em seu componente. Ambos estão relacionados também aos métodos de `life-cycle` (e podem ser sincronizados à diversos `Hooks`.).
...
- Você saberia explicar um pouco sobre como funciona a arquitetura do `Node`, se é `single-thread`/`multi-thread` e como funciona um pouco mais a fundo?
